// Headless test for functions/api/*.js with a local SQLite stand-in for D1 and an in-memory R2.
...
const load = async f => { const tmp = path.join(os.tmpdir(), 'bl-' + f.replace('.js', '.mjs')); fs.copyFileSync(path.join(here, '..', 'functions', 'api', f), tmp); return import(pathToFileURL(tmp).href + '?' + Date.now()); };
